...
// ── Paths ─────────────────────────────────────────────────
// O script fica em scripts/ mas os JSONs ficam na raiz do repositório
...
async function atualizarLoteria(cfg) {
  const arquivo = path.join(ROOT_DIR, cfg.arquivo);
...
async function main() {
  ...
  if (algumAtualizado) {
    ...
    fs.writeFileSync(path.join(ROOT_DIR, '.historico-atualizado'), '1');
  } else {
